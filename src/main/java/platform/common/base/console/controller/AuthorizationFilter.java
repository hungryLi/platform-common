...
import platform.common.utils.Md5Utils;
...
public class AuthorizationFilter implements Filter {
	...
	public void doFilter(ServletRequest paramServletRequest, ServletResponse paramServletResponse,
		...
		if(secretAccessKey.indexOf("ogc-") == 0){//ogc-开头的appKey需要特殊验证user-Agent
			String userAgent = request.getHeader("User-Agent");
			secretAccessKey = Md5Utils.getMd5ByStr(secretAccessKey+userAgent);
		}
		
		StringBuffer  sb = new StringBuffer();
		sb.append(requestString).append(reqURL);
		String sign=null;
		try {
		...
		if (!reqSign.equals(sign)) {
			logger.error("request sign:"+reqSign+",sign:"+sign+",Verify signature failed ");
			String resp = "{\"code\":\"999998\",\"msg\":\" Verify signature failed  \"}";// SecretAccessKeyID有误返回码
			response.getWriter().write(resp);
			response.getWriter().flush();
			return;
